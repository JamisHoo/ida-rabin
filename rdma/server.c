...
void init() {
    ...
    for (i = 0; i < COLUMN / 4 * 5; ++i) {
        output[i] = output[0] + DATA_SIZE / COLUMN * i;
        row[i] = i;
    }
...
int main(int argc, char** argv) {
    ...
    for (i = 0; i < NUM_CLIENTS; ++i) {
        ...
        mr[i] = ibv_reg_mr(pd[i], data + DATA_SIZE / NUM_CLIENTS * i, 
                           ...
                           IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_READ| IBV_ACCESS_REMOTE_WRITE); // TODO remove remote write
        assert(mr[i]);
        ...
        sge[i].addr = (uintptr_t)(data + DATA_SIZE / NUM_CLIENTS * i);
        sge[i].length = DATA_SIZE / NUM_CLIENTS;
        sge[i].lkey = mr[i]->lkey;
